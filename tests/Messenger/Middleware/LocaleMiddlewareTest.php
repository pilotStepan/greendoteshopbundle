...
class LocaleMiddlewareTest extends TestCase
{
    ...
    public function testReceiveSideWithNoLocaleStampCallsNextWithoutSwitchingLocale(): void
    {
        $spy = new class implements LocaleAwareInterface {
            public array $seenLocales = [];
            private string $locale = 'cs';
        ...
        // Received, but never stamped (e.g. dispatched before this middleware existed).
        $envelope = new Envelope(new \stdClass(), [new ReceivedStamp('async')]);

        $nextCalled = false;
        $stack = $this->createStackReturning(function (Envelope $received) use (&$nextCalled) {
            $nextCalled = true;
        ...
        $this->assertTrue($nextCalled, 'handling must still proceed without a LocaleStamp');
        $this->assertSame([], $spy->seenLocales, 'locale must never be switched when there is nothing to restore to');
    }

    /**
     * runWithLocale() restores the original locale in a finally block; the
     * middleware must not swallow that guarantee, or a handler exception on
     * the worker would permanently leave the process in the wrong locale for
     * every message handled afterwards.
     */
    public function testReceiveSideRestoresLocaleEvenWhenNextHandlerThrows(): void
    {
        $localeSwitcher = new LocaleSwitcher('cs', []);
        $middleware = new LocaleMiddleware($localeSwitcher);
        ...
        $stack = $this->createStackReturning(function () {
        ...
        $this->assertSame('cs', $localeSwitcher->getLocale(), 'locale must be restored even after a handler exception');
    }
