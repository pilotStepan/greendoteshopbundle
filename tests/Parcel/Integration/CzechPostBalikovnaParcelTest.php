...
class CzechPostBalikovnaParcelTest extends TestCase
{
    ...
    private function makeService(MockHttpClient $httpClient, bool $enabled = true, string $environment = 'test', string $iso = 'CZK'): CzechPostBalikovnaParcel
    {
    ...
    public function testCreateParcel_nonCzkPurchase_throwsPermanentParcelExceptionWithoutCallingApi(): void
    {
        ...
        try {
            $service->createParcel($this->makePurchase(
                $this->makeTransportation('c2VjcmV0'),
                $this->makeBranch(),
                currencyIso: 'EUR',
            ));
        ...
        $this->assertFalse($called, 'Czech Post Balíkovna must not be called for a non-CZK purchase');
    }
